...
class QueryLogRepositoryTest(unittest.TestCase):
    ...
    @patch.object(query_log_repository, "SessionLocal")
    def test_saves_query_log_and_returns_id(self, session_local, query_log):
        db = MagicMock()
    ...
    def test_returns_none_when_save_fails(self, logger_exception, session_local):
        session_local.return_value.__enter__.side_effect = RuntimeError("db unavailable")
